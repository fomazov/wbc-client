...
(function($){
    ...
    //
    //$headerTop.find('a, .navigation-enter').on('click', function(e){
    //    e.preventDefault();
    //    var that = $(this),
    //        attrBg = that.parent().data('bg'),
    //        $headerEl = $('.header');
    //
    //
    //    if( that.hasClass(activeClass) ){
    //        that.removeClass(activeClass);
    //        $headerEl.attr('data-style-bg', 'default');
    //    }
    //    else{
    //        $headerTop.find('.' + activeClass).removeClass(activeClass);
    //        that.addClass(activeClass);
    //        $headerEl.attr('data-style-bg', attrBg);
    //    }
    //
    //});
    //
    //var $checkedEl = $('.header--top').find('.__checked');
    //
    //
    //// add data attribute to header
    //if($checkedEl.length > 0){
    //    $('.header').attr('data-status', $checkedEl.parent().data('bg') );
    //}
...
(function(){
    ...
    window.onresize = function(event) {
        if( document.documentElement.clientWidth <= 1024) document.body.classList.remove('menu-pin');
    };
})();
